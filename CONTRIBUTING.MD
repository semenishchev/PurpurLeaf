Contributing to PurpurLeaf
==========================
Thank you for contributing!. We are usually
...
- `purpurleaf-api` - Modifications to the Bukkit and Paper API;
- `purpurleaf-server` - Modifications to the Minecraft server and API implementation.
...
3. cd into `purpurleaf-server` for server changes, and `purpurleaf-api` for API changes.
...
`purpurleaf-server` and `purpurleaf-api` aren't git repositories in the traditional sense:
...
1. Modify `purpurleaf-server` and/or `purpurleaf-api` with the appropriate changes;
...
1. If your PR creates new patches instead of modifying existing ones, in both the `purpurleaf-server` and `purpurleaf-api` directories, ensure your newly-created patch is the last commit by either:
...
If you add a message to your commit in the `purpurleaf-server`/`purpurleaf-api`
...
- If you later need to use the purpurleaf-api, you might want to remove the jar
...
would like to make changes to a file that isn't present in `purpurleaf-server`'s
...
1. Identify the name(s) of the file(s) you want to import.
    - A complete list of all possible file names can be found at
      `./purpurleaf-server/.gradle/caches/paperweight/mc-dev-sources/net/minecraft/`. You might find
